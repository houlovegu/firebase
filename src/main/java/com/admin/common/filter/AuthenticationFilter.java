...
public class AuthenticationFilter extends OncePerRequestFilter {
    ...
    private void filter(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) {
        try {
            ...
            // 放行swagger
            if (StrUtil.containsAny(requestURI, "/favicon.ico","/user/reset","/user/register","/user/login","/auth/getCaptcha","/auth/token","/doc.html","webjars","swagger-resources","/v2/api-docs")) {
